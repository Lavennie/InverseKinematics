...
public class IK : MonoBehaviour
{
    ...
    {
        Vector3 dirToEnd = endEffector.transform.position - bones[i].transform.position;
        ...
        {
            // law of cosines
            float angle1 = Mathf.Acos((bones[i].length * bones[i].length + distToEnd * distToEnd - 
            ...
            // law of sines
            /*float angle2 = Mathf.Asin(distToEnd * Mathf.Sin(angle1) / bones[i + 1].length);
            if (distToEndSquared < bones[i].length * bones[i].length + bones[i + 1].length * bones[i + 1].length)
            {
                angle2 = Mathf.PI - angle2;
            }*/
            bones[i].transform.forward = Quaternion.Euler(angle1, 0, 0) * dirToEnd;
            //bones[i + 1].transform.forward = Quaternion.Euler((-angle1 + angle2) * Mathf.Rad2Deg, 0, 0) * dirToEnd;
        }
    }
