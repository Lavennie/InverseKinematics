...
namespace InverseKinematics1
{
    ...
    public struct AngleRegion
    {
        ...
        public float ClampInside(float angle)
        {
            return Mathf.Clamp(angle, this.min, this.max);
        }

        public void Debug(Vector3 position, Vector3 dir, Vector3 axis)
        {
            Debug(position, dir, axis, Color.white);
        }
        public void Debug(Vector3 position, Vector3 dir, Vector3 axis, Color color)
        {
            UnityEngine.Debug.DrawRay(position, Quaternion.AngleAxis(min, axis) * dir * 0.5f, color, 0.01f);
            UnityEngine.Debug.DrawRay(position, Quaternion.AngleAxis(max, axis) * dir * 0.5f, color, 0.01f);
        }
